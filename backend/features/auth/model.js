...
export const getLoginCredentials = async (user) => {
    try{
        ...
        const query = 'SELECT u.user_id, u.password_hash, u.user_role, t.team_id FROM users u LEFT JOIN team_members t ON t.user_id = u.user_id WHERE u.email = $1';
        const result = await pool.query(query, [email]); 
        if (result.rows.length === 0) {
            return null; // No user found with the provided credentials
        }
        const isUserValid = await argon2.verify(result.rows[0].password_hash, password_hash); //verifies the provided password against the stored hash using argon2
        if (!isUserValid) {
